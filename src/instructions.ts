...
Why Pulse exists. Running a high-quality, efficient software business at scale demands visibility that single-source dashboards (just Jira, just GitHub, just a survey tool) cannot give you. Efficiency, quality, and accountability interact, and the real picture only emerges when engineering output, QA outcomes, planning accuracy, code health, and team sentiment are reviewed together. In an era where AI is raising expectations on engineering throughput, Pulse is how Studio Graphene proves and improves its delivery. Pulse is also being developed as a SaaS product for early-stage SaaS businesses and CTO-led product teams of roughly 20 to 80 engineers, so answers should work for internal Studio Graphene projects and external tenants alike.
...
export const TOOL_DESCRIPTIONS: Record<string, string> = {
	...
	pulse_find_user:
		"Case-insensitive substring search over names and emails; returns user UUIDs. Use this first whenever the user names a person ('Sarah's metrics', 'add Josh to the project'). Much faster than paging pulse_list_users.",
